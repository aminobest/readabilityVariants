...
public class Kiosk {
	...
	private static boolean luhnCheckFails(String cardNumber) {
	...
	public Boolean IsLengthWithinRange(String cardNumber) {
	...
	public ResponseObject verify(CreditCard creditCard) {
		...
		if (IsLengthWithinRange(cardNumber)) {
		...
		if (!CreditCardCompany.isCompanyUnknown(cardNumber)) {
